...
class Reward(object):
    def __init__(self, lstm, update_rate, featloader):
        self.lstm = lstm
        ...
        self.f_trans_unit = self.create_feat_trans_unit()

        #####################################################################################################
        # placeholder definition
        self.f = tf.placeholder(tf.float32, shape=[self.batch_size,
                                                   self.feat_size])
        ...
        feat_emb = self.f_trans_unit(self.f)
        self.h0 = self.g_recurrent_unit(feat_emb, self.h0)
    ...
    def get_reward(self, sess, input_x, rollout_num, discriminator):
        rewards = []
        in_f = self.featloader.get_feat(map(str, input_x[:, 0]))
        in_x = input_x[:, 1:]
        for i in range(rollout_num):
            for given_num in range(1, len(in_x[0])):
                feed = {self.f:in_f, self.x: in_x, self.given_num: given_num}
                samples = sess.run(self.gen_x, feed)
                feed = {discriminator.input_f: in_f, discriminator.input_x: samples}
            ...
            feed = {discriminator.input_f: in_f, discriminator.input_x: in_x}
            ypred_for_auc = sess.run(discriminator.ypred_for_auc, feed)
            ypred = np.array([item[1] for item in ypred_for_auc])
            if i == 0:
                rewards.append(ypred)
            else:
                rewards[(len(in_x[0])-1)] += ypred
    ...
    def create_feat_trans_unit(self):
        self.Wt = tf.identity(self.lstm.Wt)
        self.bt = tf.identity(self.lstm.bt)

        def unit(raw_feat):
            o_feat = tf.matmul(raw_feat, self.Wt) + self.bt
            return o_feat

        return unit
    
    def update_feat_trans_unit(self):
        self.Wt = self.update_rate * self.Wt + (1 - self.update_rate) * tf.identity(self.lstm.Wt)
        self.bt = self.update_rate * self.bt + (1 - self.update_rate) * tf.identity(self.lstm.bt)
    ...
    def update_params(self):
        ...
        self.f_trans_unit = self.update_feat_trans_unit()