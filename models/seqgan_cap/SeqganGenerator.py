...
class Generator(object):
    def __init__(self, num_vocabulary, batch_size, emb_dim, hidden_dim,
                 sequence_length, start_token, featloader,
        ...
        with tf.variable_scope('generator'):
            ...
            self.f_trans_unit = self.create_feat_trans_unit(self.g_params)
    ...
    def generate(self, sess):
        imgs, feats = self.featloader.sample(self.batch_size)
        captions = sess.run(self.gen_x, feed_dict={self.f: feats})
        outputs = np.insert(captions, 0, imgs, axis=1)
        return outputs

    def pretrain_step(self, sess, x):
        outputs = sess.run([self.pretrain_updates, self.pretrain_loss], feed_dict={self.f:x[0], self.x: x[1]})
        return outputs
    ...
    def create_feat_trans_unit(self, params):
        self.Wt = tf.Variable(self.init_matrix([self.feat_size, self.emb_dim]))
        self.bt = tf.Variable(self.init_matrix([self.emb_dim]))
        params.extend([self.Wt, self.bt])
